...
class TemplateNinjaNewFileCommand(sublime_plugin.WindowCommand):
	...
	def run(self):
		...
		#self.window.show_input_panel("Environment", "dev", self.on_file_name_entered, None, None)
		#self.create_file('test.php')
	...
	def show_template_selection(self, filename):
		...
		templates = self.find_templates_for_scope(scope)
		...
		for t in self.templates:
			...
			options.append([ description, scope ])
	...
	def find_templates_for_scope(self, scope):
		...
		for f in files:
			...
			self.output("f: " + f)
			self.output("selected_path" + self.selected_path)
			...
			try:
				...
				file_scope = template.find("scope").text
	...
	def get_setting(self, key):
		...
		if settings and settings.has('SuperNewFile') and key in settings.get('SuperNewFile'):
		    # Get project-specific setting
		    results = settings.get('SuperNewFile')[key]
		else:
		    # Get user-specific or default setting
		    settings = sublime.load_settings('SuperNewFile.sublime-settings')
